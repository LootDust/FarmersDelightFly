# Farmer's Delight Fly

<img src="https://i.imgur.com/aDELzhM.png" width="50%">

### Overview

**Farmer's Delight Fly** is an unofficial port of famous Minecraft mod [Farmer's Delight](https://github.com/vectorwing/FarmersDelight).

This project is aiming to bring funny stuff in FD back to 26+ NeoForged Minecraft.

### Mod version support

✅ - Still active  
🔨 - Basic maintaining, no more developing  
❌ - Stop maintaining

| Minecraft version | NeoForge version | Mod Support |
|:-----------------:|:----------------:|:-----------:|
|       26.1        |        -         |      ❌      |
|       26.2        |        -         |      ❌      |
|       26.3        |  26.3.0.7-beta+  |      ✅      |

### Credits

[Vectorwing](https://github.com/vectorwing) - Original author of Farmer's Delight

And all the contributors to the original Farmer's Delight project